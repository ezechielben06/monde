...
import { Sparkles, Activity, User, Home, Crown, LogOut, Settings } from 'lucide-react';
    ...
    <nav className="fixed bottom-0 left-0 right-0 z-[100] bg-black/90 backdrop-blur-2xl border-t border-white/5 md:top-0 md:bottom-auto md:border-b md:border-t-0 shadow-[0_40px_100px_rgba(0,0,0,0.9)]">
      <div className="max-w-[1600px] mx-auto px-12 md:px-20 h-28 flex items-center justify-between">
        <Link href="/" className="flex items-center gap-6 group">
          ...
          <span className="font-serif font-bold text-4xl tracking-tighter luxury-gold-gradient hidden sm:block">B-right</span>
        </Link>
        ...
        <div className="flex flex-1 justify-around md:justify-end md:gap-16 items-center h-full">
          ...
          <div className="hidden md:flex items-center gap-6 pl-10 border-l border-white/5 h-12">
...
function NavLink({ href, icon, label, active }: { href: string; icon: React.ReactNode; label: string, active?: boolean }) {
  return (
    <Link href={href} className={cn(
      ...
      <span className="text-[9px] md:text-[11px] font-bold uppercase tracking-[0.4em]">{label}</span>
    </Link>
  );
}
